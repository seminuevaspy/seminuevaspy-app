...
with tab1:
    ...
    with st.form("formulario_ventas", clear_on_submit=True):
        monto = st.number_input("Monto de la venta (₲)", min_value=0, step=10000, format="%d")
        ...
        if submit:
            if monto > 0:
                exito = db.registrar_venta(monto, metodo, clienta, vendedora)
                if exito:
                    st.success(f"✅ Venta de ₲ {monto:,} guardada correctamente.")
                else:
                    st.error("❌ Hubo un error al guardar la venta.")
            else:
                st.warning("⚠️ Por favor, ingresa un monto válido mayor a 0.")
...
with tab2:
    ...
    if not df_ventas.empty:
        ...
        # Fila 1: La salud general de la tienda
        ...
        # Fila 2: El equipo con colores personalizados
        c4, c5, c6 = st.columns(3)
        
        # CAJA LILA - ROMINA
        c4.markdown(f"""
        <div style="background-color: rgba(155, 89, 182, 0.15); border: 1px solid rgba(155, 89, 182, 0.5); padding: 15px; border-radius: 8px;">
            <p style="margin:0px; font-size: 14px; font-weight: 500;">Comisión Romina (7%)</p>
            <h3 style="margin:0px; margin-top:10px; color: #D7BDE2;">₲ {comision_romina:,.0f}</h3>
        </div>
        """, unsafe_allow_html=True)
        
        # CAJA ROSA - PAMELA
        c5.markdown(f"""
        <div style="background-color: rgba(232, 67, 147, 0.15); border: 1px solid rgba(232, 67, 147, 0.5); padding: 15px; border-radius: 8px;">
            <p style="margin:0px; font-size: 14px; font-weight: 500;">Comisión Pamela (7%)</p>
            <h3 style="margin:0px; margin-top:10px; color: #FF9EE2;">₲ {comision_pamela:,.0f}</h3>
        </div>
        """, unsafe_allow_html=True)
        
        # CAJA AZUL - DAMIÁN
        c6.markdown(f"""
        <div style="background-color: rgba(52, 152, 219, 0.15); border: 1px solid rgba(52, 152, 219, 0.5); padding: 15px; border-radius: 8px;">
            <p style="margin:0px; font-size: 14px; font-weight: 500;">Comisión Damián (3%)</p>
            <h3 style="margin:0px; margin-top:10px; color: #AED6F1;">₲ {comision_damian:,.0f}</h3>
        </div>
        """, unsafe_allow_html=True)
...
with tab3:
    ...
    if not df_ventas_admin.empty:
        st.dataframe(df_ventas_admin, use_container_width=True, hide_index=True)
